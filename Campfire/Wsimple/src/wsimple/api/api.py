...
from .errors import WealthsimpleDownException
...
class Wsimple:
    ...
    def __init__(
        ...
        if self.public_mode:
            ...
            if "x-wealthsimple-otp-required" in r.headers:
                ...
                try:
                    otp_headers = (
                        r.headers["x-wealthsimple-otp"].replace(" ", "").split(";")
                    )
    ...
    def get_websocket_uri(self, tokens=None):
        ...
        return (
            "wss://trade-service.wealthsimple.com/websocket?ticket={}&version=2".format(
                res.ticket
            )
        )
    ...
    def is_operational(self):
        ...
        Returns:
            bool: status of wealthsimple where False is not working and True is working
        """
        ws_status = self.current_status()
        indicator, description = ws_status.status.indicator, ws_status.status.description
        if (indicator == "none" and description == "All Systems Operational"):
            return True
        else:     
            raise WealthsimpleDownException(f"It seems that Wealthsimple is down ( {indicator} , {description})")
